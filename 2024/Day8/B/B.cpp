    ...
    for (auto v : mp) {
        ...
        for (int i = 0; i < sz; i++) {
            for (int j = i + 1; j < sz; j++) {
                ...
                pair<int, int> f = {v.second[i].first, v.second[i].second};
                while (ok(f.first, f.second, n, m)) {
                    marked[f.first][f.second] = true;
                    f = {f.first + diff.first, f.second + diff.second};
                }
                pair<int, int> w = {v.second[j].first, v.second[j].second};
                while (ok(w.first, w.second, n, m)) {
                    marked[w.first][w.second] = true;
                    w = {w.first - diff.first, w.second - diff.second};
                }
            }
        }
    }
